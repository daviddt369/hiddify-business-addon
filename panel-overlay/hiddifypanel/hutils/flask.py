...
def get_request_hostname() -> str:
    ...
    return parsed.hostname or request.host


def get_request_host_for_links() -> str:
    return request.host


def get_request_scheme_for_links() -> str:
    if os.environ.get("HIDDIFY_LOCAL_SQLITE", "").lower() in {"1", "true", "yes", "on"}:
        return "http"
