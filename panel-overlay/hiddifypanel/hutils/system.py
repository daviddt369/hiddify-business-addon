...
def _get_hiddify_root() -> str:
    return os.environ.get("HIDDIFY_MANAGER_DIR", "/opt/hiddify-manager/")


def _get_load_average(num_cpus: int) -> list[float]:
    if hasattr(os, "getloadavg"):
        return [avg / max(num_cpus, 1) for avg in os.getloadavg()]
    cpu_ratio = psutil.cpu_percent(interval=0.1) / 100
    return [cpu_ratio, cpu_ratio, cpu_ratio]
...
def system_stats() -> dict:
    ...
    # Disk usage (in GB)
    disk_stats = psutil.disk_usage(_get_disk_root())
    disk_used = disk_stats.used / 1024**3
    disk_total = disk_stats.total / 1024**3

    hiddify_used = get_folder_size(_get_hiddify_root()) / 1024**3
    ...
    load_avg = _get_load_average(num_cpus)
