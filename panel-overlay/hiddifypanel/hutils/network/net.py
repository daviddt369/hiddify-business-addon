...
def get_ips(version: Literal[4, 6] | None = None) -> List[Union[ipaddress.IPv4Address, ipaddress.IPv6Address]]:
    ...
    # send request
    try:
        ip = urllib.request.urlopen(f'https://v{version}.ident.me/', timeout=IDENT_ME_TIMEOUT).read().decode('utf8')
        if ip:
...
def get_random_domains(count: int = 1, retry: int = 3) -> List[str]:
    try:
        ...
        data_ir = requests.get(irurl, timeout=EXTERNAL_API_TIMEOUT).json()
        ...
        print('Error, getting random domains... ', e, 'retrying...', retry, file=sys.stderr)
        if retry <= 0:
            ...
            print('Error, using default domains', file=sys.stderr)
