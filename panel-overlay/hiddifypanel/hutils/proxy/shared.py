...
def get_ssh_hostkeys(hconfigs, dojson=False) -> list[str] | str:
    host_keys = [
        ...
        hconfigs.get(ConfigEnum.ssh_host_ed25519_pub, ""),
        hconfigs.get(ConfigEnum.ssh_host_ecdsa_pub, ""),
        hconfigs.get(ConfigEnum.ssh_host_rsa_pub, ""),
    ]
    if dojson:
...
def make_proxy(hconfigs: dict, proxy: Proxy, domain_db: Domain, phttp=80, ptls=443, pport: int | None = None) -> dict:
    ...
    base = {
        **domain_data,
        ...
        'fingerprint': hconfigs.get(ConfigEnum.utls, "chrome"),
