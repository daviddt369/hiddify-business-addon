...
def _inactive_subscription_message(user: User) -> str:
...
def make_v2ray_configs(domains: list[Domain], user: User, expire_days: int, ip_debug=None) -> str:
    ...
    if not user.is_active:
        if hconfig(ConfigEnum.lang) == 'fa' and user.inactive_reason not in {"max_ips", "usage_limit", "expired"}:
            res.append('trojan://1@1.1.1.1#' + hutils.encode.url_encode('✖ بسته شما به پایان رسید'))
        else:
            res.append('trojan://1@1.1.1.1#' + hutils.encode.url_encode(f'✖ {_inactive_subscription_message(user)}'))
        return "\n".join(res)
