...
class Domain(db.Model):
    ...
    @staticmethod
    def _sort_key(model: 'Domain'):
        value = (model.domain or "").strip().lower()
        try:
        ...
        is_sslip = "sslip.io" in value
        is_direct = model.mode == DomainType.direct
        return (
            1 if is_ip else 0,
            1 if is_sslip else 0,
    ...
    def get_panel_link(cls, child_id: int | None = None) -> str | None:
        ...
        if not domains:
            return None
        return sorted(domains, key=Domain._sort_key)[0].domain
    ...
    def get_domains(cls, always_add_ip=False, always_add_all_domains=False) -> List['Domain']:
        ...
        if len(domains) == 0 and request:
            domains = [Domain(domain=hutils.flask.get_request_hostname())]  # type: ignore
        if len(domains) == 0 or always_add_ip:
            domains += [Domain(domain=hutils.network.get_ip_str(4))]  # type: ignore
        return sorted(domains, key=Domain._sort_key)
