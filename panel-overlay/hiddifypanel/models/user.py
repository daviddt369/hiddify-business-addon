...
from hiddifypanel.access import compute_user_active, compute_user_inactive_reason
...
class UserDetail(db.Model):
    ...
    @property
    def devices(self):
        return [] if not self.connected_devices else [d for d in self.connected_devices.split(",") if d]
...
class User(BaseAccount):
    ...
    def remove(self, commit=True) -> None:
        ...
        from hiddifypanel.models.commercial import CommercialSubscription
        user_driver.remove_client(self)
        inspector = inspect(db.engine)
        tables = set(inspector.get_table_names())
        UserDetail.query.filter(UserDetail.user_id == self.id).delete(synchronize_session=False)
        CommercialSubscription.query.filter(CommercialSubscription.user_id == self.id).delete(synchronize_session=False)
        if "report" in tables:
            from hiddifypanel.models.report import Report
            Report.query.filter(Report.user_id == self.id).delete(synchronize_session=False)
        db.session.delete(self)
        if commit:
            db.session.commit()

    def apply_commercial_plan(self):
    ...
    def add_or_update(cls, commit: bool = True, **data):
        ...
        if data.get('max_ips') is not None:
            dbuser.max_ips = max(1, min(int(data['max_ips']), 10))

        if data.get('plan_id') is not None:
            dbuser.plan_id = data['plan_id']
