...
def get_hconfigs(child_id: int | None = None, json=False) -> dict:
    ...
    def _cfg_key(raw_key):
        if hasattr(raw_key, "type"):
            return raw_key
        if raw_key is None:
            return ConfigEnum.not_found
        try:
            return ConfigEnum(str(raw_key))
        except BaseException:
            return ConfigEnum.not_found

    bool_values = {}
    for u in BoolConfig.query.filter(BoolConfig.child_id == child_id).all():
        key_obj = _cfg_key(u.key)
        if key_obj.type == bool:
            bool_values[f'{u.key}' if json else key_obj] = u.value
    ...
    for u in StrConfig.query.filter(StrConfig.child_id == child_id).all():
        ...
        if key_obj.type == bool:
            continue
        value = int(u.value) if key_obj.type == int and u.value is not None else u.value
        str_values[f'{u.key}' if json else key_obj] = value

    return {**bool_values, **str_values}
