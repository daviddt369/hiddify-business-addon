...
def _is_local_sqlite_dev() -> bool:
    return (
        ...
        and os.environ.get("HIDDIFY_LOCAL_SQLITE", "").lower() in {"1", "true", "yes", "on"}
    )


def _safe_get_external_ip() -> str:
    return "127.0.0.1"


def _safe_get_random_domain(default_value: str) -> str:
    return default_value


def _skip_bootstrap_domain_probes() -> bool:
    return os.environ.get("HIDDIFY_SKIP_BOOTSTRAP_DOMAIN_PROBES", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _bootstrap_random_domains(count: int, retry: int = 0) -> list[str]:
    if _skip_bootstrap_domain_probes():
        if count <= len(_BOOTSTRAP_DOMAIN_FALLBACKS):
            return random.sample(_BOOTSTRAP_DOMAIN_FALLBACKS, count)
        pool = _BOOTSTRAP_DOMAIN_FALLBACKS.copy()
        while len(pool) < count:
            pool.extend(_BOOTSTRAP_DOMAIN_FALLBACKS)
        return pool[:count]
    return hutils.network.get_random_domains(count, retry=retry)


def _ensure_local_dev_unique_domains():
    unique_domain_defaults = {
        ConfigEnum.decoy_domain: "www.cloudflare.com",
        ConfigEnum.telegram_fakedomain: "www.wikipedia.org",
        ConfigEnum.ssfaketls_fakedomain: "www.python.org",
        ConfigEnum.shadowtls_fakedomain: "www.github.com",
        ConfigEnum.ssr_fakedomain: "www.microsoft.com",
    }

    seen_values = set()
    changed = False
    for key, fallback_value in unique_domain_defaults.items():
        current_value = hconfig(key)
        if not current_value or current_value in seen_values or current_value == "example.com":
            set_hconfig(key, fallback_value, commit=False)
...
def _ensure_local_dev_valid_branding():
    valid_local_branding_site = "http://127.0.0.1.nip.io/"
    current_branding_site = hconfig(ConfigEnum.branding_site)
    branding_site_pattern = r"()|(http(s|)://([A-Za-z0-9\-\.]+\.[a-zA-Z]{2,})/?.*)"

    if not current_branding_site or not re.fullmatch(branding_site_pattern, current_branding_site):
        set_hconfig(ConfigEnum.branding_site, valid_local_branding_site, commit=False)
        db.session.commit()


def _init_local_sqlite_db():
    db.create_all()

    child = Child.by_id(0)
    if child is None:
        child = Child(id=0, unique_id=str(uuid.uuid4()), name="Root", mode=ChildMode.virtual)
    ...
    AdminUser.get_super_admin()

    if not User.query.first():
        db.session.add(User(name="default", usage_limit_GB=3000, package_days=3650, mode=UserMode.monthly, added_by=1))
        db.session.commit()

    if not Domain.query.first():
        local_ip = _safe_get_external_ip()
        db.session.add(Domain(domain=local_ip, mode=DomainType.direct, child_id=0))
        db.session.add(Domain(domain="127.0.0.1.nip.io", mode=DomainType.direct, child_id=0))
        db.session.commit()

    default_configs = {
        ConfigEnum.db_version: latest_db_version(),
        ConfigEnum.admin_secret: str(uuid.uuid4()),
        ConfigEnum.lang: "en",
        ConfigEnum.admin_lang: "en",
        ConfigEnum.first_setup: False,
        ConfigEnum.http_ports: "5000",
        ConfigEnum.tls_ports: "5443",
        ConfigEnum.decoy_domain: _safe_get_random_domain("www.cloudflare.com"),
        ConfigEnum.proxy_path: hutils.random.get_random_string(12, 18),
        ConfigEnum.proxy_path_admin: "admin",
        ConfigEnum.proxy_path_client: "client",
        ConfigEnum.core_type: "xray",
        ConfigEnum.utls: "chrome",
        ConfigEnum.panel_mode: PanelMode.standalone,
        ConfigEnum.log_level: LogLevel.INFO,
        ConfigEnum.unique_id: str(uuid.uuid4()),
        ...
        ConfigEnum.is_parent: False,
        ConfigEnum.parent_panel: "",
        ConfigEnum.license: "",
        ConfigEnum.domain_fronting_domain: "",
        ConfigEnum.branding_title: "Hiddify Commercial Dev",
        ConfigEnum.branding_site: "http://127.0.0.1.nip.io/",
        ConfigEnum.branding_freetext: "Local commercial fork test environment",
        ConfigEnum.shared_secret: str(uuid.uuid4()),
        ConfigEnum.default_useragent_string: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/135.0.0.0 Safari/537.36",
        ConfigEnum.telegram_adtag: "",
        ConfigEnum.telegram_fakedomain: _safe_get_random_domain("www.wikipedia.org"),
        ConfigEnum.ssfaketls_fakedomain: _safe_get_random_domain("www.python.org"),
        ConfigEnum.shadowtls_fakedomain: _safe_get_random_domain("www.github.com"),
        ConfigEnum.ssr_fakedomain: _safe_get_random_domain("www.microsoft.com"),
        ConfigEnum.shadowsocks2022_method: "2022-blake3-aes-128-gcm",
        ConfigEnum.shadowsocks2022_port: "443",
        ConfigEnum.ssh_server_port: "2222",
        ConfigEnum.wireguard_port: "51820",
        ConfigEnum.wireguard_ipv4: "172.20.20.0",
        ConfigEnum.wireguard_ipv6: "fdfe:dcba:9876::",
        ConfigEnum.wireguard_public_key: "local-dev-wireguard-public-key",
        ConfigEnum.wireguard_noise_trick: "0-0",
        ConfigEnum.hysteria_port: "8443",
        ConfigEnum.hysteria_up_mbps: "100",
        ConfigEnum.hysteria_down_mbps: "100",
        ConfigEnum.tuic_port: "9443",
        ConfigEnum.naive_port: "10443",
        ConfigEnum.mieru_tcp_ports: "30000,30001,30002,30003",
        ConfigEnum.mieru_udp_ports: "31000,31001,31002,31003",
        ConfigEnum.mux_protocol: "h2mux",
        ConfigEnum.reality_public_key: "local-dev-reality-public-key",
        ConfigEnum.reality_private_key: "local-dev-reality-private-key",
        ConfigEnum.reality_short_ids: "0123456789abcdef",
        ConfigEnum.ssh_host_rsa_pub: "",
        ConfigEnum.ssh_host_ed25519_pub: "",
        ConfigEnum.ssh_host_ecdsa_pub: "",
        ConfigEnum.path_vless: hutils.random.get_random_string(7, 15),
        ConfigEnum.path_trojan: hutils.random.get_random_string(7, 15),
        ConfigEnum.path_vmess: hutils.random.get_random_string(7, 15),
        ConfigEnum.path_ss: hutils.random.get_random_string(7, 15),
        ConfigEnum.path_grpc: hutils.random.get_random_string(7, 15),
        ConfigEnum.path_tcp: hutils.random.get_random_string(7, 15),
        ConfigEnum.path_ws: hutils.random.get_random_string(7, 15),
        ConfigEnum.path_httpupgrade: hutils.random.get_random_string(7, 15),
        ConfigEnum.path_xhttp: hutils.random.get_random_string(7, 15),
    }
    for key, value in default_configs.items():
        add_config_if_not_exist(key, value, child_id=0)

    default_bools = {
        ConfigEnum.firewall: False,
        ConfigEnum.netdata: False,
        ConfigEnum.block_iran_sites: False,
        ConfigEnum.allow_invalid_sni: True,
        ConfigEnum.kcp_enable: False,
        ConfigEnum.auto_update: False,
        ConfigEnum.speed_test: False,
        ConfigEnum.only_ipv4: False,
        ConfigEnum.vmess_enable: True,
        ConfigEnum.http_proxy_enable: True,
        ConfigEnum.telegram_enable: False,
        ConfigEnum.ssfaketls_enable: False,
        ConfigEnum.shadowtls_enable: False,
        ConfigEnum.ssr_enable: False,
        ConfigEnum.domain_fronting_tls_enable: False,
        ConfigEnum.domain_fronting_http_enable: False,
        ConfigEnum.tuic_enable: False,
        ConfigEnum.hysteria_enable: False,
        ConfigEnum.shadowtls_enable: False,
        ConfigEnum.xhttp_enable: False,
        ConfigEnum.httpupgrade_enable: True,
        ConfigEnum.grpc_enable: True,
        ConfigEnum.ws_enable: False,
        ConfigEnum.h2_enable: True,
        ConfigEnum.reality_enable: False,
        ConfigEnum.ssh_server_enable: False,
        ConfigEnum.wireguard_enable: False,
        ConfigEnum.tls_ech_enable: False,
        ConfigEnum.mux_enable: False,
        ConfigEnum.mux_padding_enable: False,
        ConfigEnum.mux_brutal_enable: False,
        ConfigEnum.hysteria_obfs_enable: False,
        ConfigEnum.sub_full_singbox_enable: True,
        ConfigEnum.sub_singbox_ssh_enable: True,
        ConfigEnum.sub_full_xray_json_enable: True,
        ConfigEnum.sub_full_links_enable: True,
        ConfigEnum.sub_full_links_b64_enable: True,
        ConfigEnum.sub_full_clash_enable: True,
        ConfigEnum.sub_full_clash_meta_enable: True,
        ConfigEnum.business_enabled: False,
    }
    for key, value in default_bools.items():
        add_config_if_not_exist(key, value, child_id=0)

    _ensure_local_dev_unique_domains()
    _ensure_local_dev_valid_branding()

    if Proxy.query.count() == 0:
        db.session.bulk_save_objects(list(get_proxy_rows_v1()))

    db.session.commit()
    logger.warning("Initialized local sqlite development database for panel testing.")
...
def _v131(child_id):
    add_config_if_not_exist(ConfigEnum.telegram_webhook_domain, "")
    add_config_if_not_exist(ConfigEnum.telegram_payment_provider_token, "")
    add_config_if_not_exist(ConfigEnum.support_url, "")
...
def _v136(child_id):
    ...
    add_config_if_not_exist(ConfigEnum.commercial_drop_bittorrent, True)
...
def _v42():
    ...
    for k in [ConfigEnum.telegram_fakedomain, ConfigEnum.ssfaketls_fakedomain, ConfigEnum.shadowtls_fakedomain]:
        if not hconfig(k):
            rnd_domains = _bootstrap_random_domains(1)
...
def _v1():
    external_ip = hutils.network.get_ip_str(4, retry=1) or "127.0.0.1"
    rnd_domains = _bootstrap_random_domains(5, retry=0)
    decoy_domain = "bbc.com" if _skip_bootstrap_domain_probes() else hutils.network.get_random_decoy_domain()
...
def init_db():
    if _is_local_sqlite_dev():
        return _init_local_sqlite_db()
