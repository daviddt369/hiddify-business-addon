...
def init_app(app: APIFlask):
    ...
    def internal_server_error(e):
        ...
        if e.status_code == 500:
            issue_link = hutils.github_issue.generate_github_issue_link_for_500_error(e, traceback.format_exc())

            has_update = hutils.utils.is_panel_outdated()
            last_version = hiddify.get_latest_release_version('hiddifypanel')

            return render_template('500.html', error=e, trace="", has_update=has_update, last_version=last_version, issue_link=issue_link), 500
    ...
    def base_middleware():
        ...
        if request.endpoint == 'static' or request.endpoint == "videos":
            return
        is_tg_webhook = request.path.endswith("/api/v2/tgbot/")

        if g.user_agent['is_bot'] and not is_tg_webhook:
            abort(400, "invalid")
