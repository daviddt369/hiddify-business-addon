...
def get_common_data(user_uuid, mode, no_domain=False, filter_domain=None):
    ...
    hostname = hutils.flask.get_request_hostname()
    link_host = hutils.flask.get_request_host_for_links()
    link_scheme = hutils.flask.get_request_scheme_for_links()
    domains, db_domain, has_auto_cdn = get_domain_information(no_domain, filter_domain, hostname)

    domain = db_domain.domain
    user: User = g.account if g.account.uuid == user_uuid else User.by_uuid(f'{user_uuid}')
    if user is None:
        abort(401, "Invalid User")
    _refresh_user_connected_devices(user)
    ...
    profile_url = hiddify.get_account_panel_link(user, link_host, is_https=link_scheme == "https")
    if has_auto_cdn and asn != 'unknown':
        profile_title += f" {asn}"

    inactive_reason = user.inactive_reason
    inactive_reason_text = _inactive_reason_text(user)
    if inactive_reason_text:
        profile_title = f'{inactive_reason_text} | {profile_title}'
...
def _refresh_user_connected_devices(user: User, child_id: int = 0) -> None:
    ips = collect_recent_ips().get(user.uuid.lower(), [])
    detail = UserDetail.query.filter(
        UserDetail.user_id == user.id,
        UserDetail.child_id == child_id,
    ).first()
    ...
    serialized = ",".join(ips)
    if detail.connected_devices != serialized:
        detail.connected_devices = serialized
        db.session.commit()


def _inactive_reason_text(user: User) -> str | None:
    reason = user.inactive_reason
    if reason == "max_ips":
        return _("IP limit exceeded")
    if reason == "usage_limit":
        return _("Traffic exhausted")
    if reason == "expired":
        return _("Time expired")
    if reason == "disabled":
        return _("Access disabled")
    return None
...
def add_headers(res, c, mimetype="text/plain"):
    ...
    profile_base_url = request.base_url.rsplit('/', 1)[0] + "/"
    profile_base_url = re.sub(r"^https?://", f"{hutils.flask.get_request_scheme_for_links()}://", profile_base_url)
    resp.headers['profile-web-page-url'] = profile_base_url
