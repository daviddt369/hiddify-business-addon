...
def init_app(app):
        ...
        app.config["PREFERRED_URL_SCHEME"] = "http" if is_fake_redis_enabled() and os.environ.get("HIDDIFY_LOCAL_SQLITE", "").lower() in {"1", "true", "yes", "on"} else "https"
        ...
        if is_fake_redis_enabled():
            logger.warning("Running with fake redis. This is intended for local development only.")
