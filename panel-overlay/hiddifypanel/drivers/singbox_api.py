...
class SingboxApi(DriverABS):
    def is_enabled(self) -> bool:
        return hconfig(ConfigEnum.core_type) == "singbox"

    def get_singbox_client(self):
        return xtlsapi.SingboxClient('127.0.0.1', 10086)

    def get_enabled_users(self):
        config_dir = os.environ.get('HIDDIFY_CONFIG_PATH')
        if not config_dir:
            return {}
        config_path = os.path.join(config_dir, "singbox", "configs", "01_api.json")
        if not os.path.exists(config_path):
            return {}
        with open(config_path, encoding="utf-8") as f:
            json_data = json.load(f)
            users = json_data.get('experimental', {}).get('v2ray_api', {}).get('stats', {}).get('users', [])
            return {u.split("@")[0]: 1 for u in users}
